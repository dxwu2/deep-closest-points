...
def apply_augmentation (lidar1, lidar2, trans):
    # randomly flip order and inverse transformation
    if (rand.uniform() > 0.5):
        temp = lidar1
        lidar1 = lidar2
        lidar2 = temp
    ...
    noise_std = 0.02
    lidar1[:,:, :3] = lidar1[:,:, :3] + rand.normal(0, noise_std, lidar1[:,:, :3].shape)
    lidar2[:,:, :3] = lidar2[:,:, :3] + rand.normal(0, noise_std, lidar2[:,:, :3].shape)
    lidar1[:,:,3] = np.linalg.norm(lidar1[:,:, :3], axis=2)
    lidar2[:,:,3] = np.linalg.norm(lidar2[:,:, :3], axis=2)
    
    # add gaussian noise to intensity
    noise_std = 0.02
    lidar1[:,:,4] = lidar1[:,:,4] + rand.normal(0, noise_std, lidar1[:,:,4].shape)
    lidar2[:,:,4] = lidar2[:,:,4] + rand.normal(0, noise_std, lidar2[:,:,4].shape)
    
#     # block out random parts of the scene
#     num_parts = 20
#     block_size = (5, 18)
#     for i in range(num_parts):
#         r = rand.randint(0, lidar1.shape[0]-block_size[0])
#         c = rand.randint(0, lidar1.shape[1]-block_size[1])
#         lidar1[r:r+block_size[0], c:c+block_size[1], :] = np.zeros((block_size[0], block_size[1], lidar1.shape[2]))
#     for i in range(num_parts):
#         r = rand.randint(0, lidar2.shape[0]-block_size[0])
#         c = rand.randint(0, lidar2.shape[1]-block_size[1])
#         lidar2[r:r+block_size[0], c:c+block_size[1], :] = np.zeros((block_size[0], block_size[1], lidar2.shape[2]))
                                                                   
    return lidar1, lidar2, trans

# Dataloader for our sim dataset
...
def test_one_epoch(args, net, test_loader):
    ...
    print('testestss:', tqdm(test_loader))
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='modelnet40', choices=['modelnet40', 'sim'], metavar='N',
                        help='dataset to use')
